@php
    $user = auth()->user();
    $isFinancial = $user->role === 'financial_affairs';
...
@else
    ...
    @if($user->role === 'student_affairs' || $isSuperAdmin)
    ...
    @if($isAdmin || $isSuperAdmin)
        ...
        <a href="{{ route('admin.login-activity') }}" class="{{ request()->routeIs('admin.login-activity') ? 'active' : '' }}">
            <i class="bi bi-shield-lock"></i> نشاط الدخول
        </a>
        @include('partials.sidebar-reports')
    @endif
@endif
